...
class Venue(db.Model):
    ...
    about_venue = db.Column(db.Boolean, nullable=False, default=False)
    about_description = db.Column(db.String(500))
...
class Show(db.Model):
    ...
    startup_time = db.Column(db.DateTime, nullable=False, default=datetime.utcnow) 

    def __repr__(self):
        return f'<Show ID: {self.id} artist_id: {self.artist_id} venue_id: {self.venue_id} start_time: {self.startup_time}>'
...
def venues():
  ...
  #       num_upcoming_shows should be aggregated based on number of upcoming shows per venue.
    data = []
    outputData = Venue.query.distinct(Venue.city, Venue.state).all()
    ...
    if form.validate():
        try:
            new_venue = Venue(
                ...
                about_venue=form.seeking_talent.data,
                about_description=form.seeking_description.data,
    ...
    if form.validate():
        try:
            new_artist = Artist(
                ...
                about_venue=form.seeking_venue.data,
                about_description=form.seeking_description.data,
            )
